...
class AdminManagement extends Component
{
    ...
    public function saveAdmin()
    {
        ...
        if ($this->userId) {
            ...
            $user->update($data);
            session()->flash('success', 'Akaun pentadbir berjaya dikemas kini.');
        } else {
            User::create($data);
            session()->flash('success', 'Akaun pentadbir baharu berjaya didaftarkan.');
        }
    ...
    {
        if ($id === auth()->id()) {
            session()->flash('error', 'Anda tidak boleh memadam akaun pentadbir anda sendiri.');
            return;
        }

        $user = User::findOrFail($id);
        $user->delete();
        session()->flash('success', 'Akaun pentadbir berjaya dipadam.');
    }
